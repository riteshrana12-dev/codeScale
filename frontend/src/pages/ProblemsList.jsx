...
const ProblemsList = () => {
  ...
  async function searchProblem(search) {
    try {
      setLoading(true); // start loading
      const response = await api.get(
        `/problems/search?text=${encodeURIComponent(search)}`,
      );
    ...
    <div className="h-screen bg-[#0a0a0f] flex flex-col overflow-hidden">
      ...
      <div className="flex-shrink-0 pt-2 pb-2 px-6 lg:px-24 bg-[#0a0a0f] z-10 border-b border-white/5">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <div className="flex items-center bg-[#0d0d1a] border border-white/10 p-1 rounded-xl shadow-inner gap-2">
            <button
              onClick={() => {
            ...
            <input
              ...
              className="w-[20em] text-white pl-2 placeholder-white/70 bg-transparent outline-none"
            />
          </div>
